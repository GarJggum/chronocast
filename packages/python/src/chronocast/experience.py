...
# Configure logger for the chronocast package
logger = logging.getLogger("chronocast")
...
class StreamTask(BaseModel):
    """
    Represents an interactive streaming task to be executed by an AI host.

    Attributes:
        host_id (Optional[str]): The ID of the AI host performing the task
        role (str): The role or persona of the AI host
        goal (str): The objective or purpose of the interactive experience
        attributes (Optional[str]): Additional attributes of the host
        context (Optional[str]): Background information for the experience
        instruction (str): Specific directions for creating the experience
        ...
        stream (bool): Whether to stream the interactive experience
    ...
    host_id: Optional[str] = Field(None, description="The ID of the AI host performing the task")
    role: str = Field(..., description="The role or persona of the AI host")
    goal: str = Field(..., description="The objective or purpose of the interactive experience")
    attributes: Optional[str] = Field(None, description="Additional attributes or characteristics of the host or expected responses")
    host: Optional[Any] = Field(None, description="The AI host associated with this task")

    # Core experience inputs
    instruction: str = Field(..., description="Specific directions for creating the experience")
    context: Optional[str] = Field(None, description="The background information or setting for the experience")
    ...
    async def _create_async(
        ...
        Args:
            host: Optional host instance
            role: Role or type of host
            goal: Task objective
            attributes: Additional host attributes
        ...
        try:
            ...
            if not role and not (host and host.role):
                raise ValueError("Role must be provided either directly or via host")
            if not goal and not (host and host.goal):
                raise ValueError("Goal must be provided either directly or via host")
            if not instruction:
                raise ValueError("Instruction is required")
            if not llm and not (host and host.llm):
                raise ValueError("LLM function must be provided either directly or via host")
            ...
            if not messages or messages[0].get("role") != "system":
                system_message = {
                    "role": "system",
                    "content": (
                        f"You are {role or (host.role if host else None)}. "
                        f"Your goal is {goal or (host.goal if host else None)}"
                        f"{' Your attributes are: ' + (attributes or (host.attributes if host else '')) if attributes or (host.attributes if host else '') else ''}"
                    ).strip(),
                }
            ...
            task_data = {
                ...
                "role": role or (host.role if host else None),
                "goal": goal or (host.goal if host else None),
                "attributes": attributes or (host.attributes if host else None),
                "context": context,
                "instruction": instruction,
                "llm": llm or (host.llm if host else None),
                "tools": tools
                or getattr(host, "tools", None)
                or None,  # Handle missing tools attribute
                "image_data": image_data,
                "temperature": temperature or (host.temperature if host else 0.7),
                "max_tokens": max_tokens or (host.max_tokens if host else 4000),
            ...
            logger.info(f"Created task for host {task.host_id or 'unknown'}")
    ...
    async def execute(
        ...
        try:
            if pre_execute:
                await pre_execute({"host_id": self.host_id})

            logger.info(f"Executing task for host {self.host_id or 'unknown'}")
            ...
            logger.error(error_msg)
            if callback:
                await callback({"type": "error", "content": error_msg, "host_id": self.host_id})
            raise
                        ...
                        if isinstance(tool_call, dict):
                            ...
                            try:
                                ...
                                if isinstance(raw_result, Exception):
                                    ...
                                    # Continue execution to let the host handle the error
                                    continue
...
def configure_logging(
    ...
    """Configure logging for the chronocast package"""
...
def default_logger(event: Dict[str, Any]) -> None:
    """Default logging callback that prints events in a human-readable format."""
